...
const ethOpts = {};
if(process.env.DEPLOY_CHAIN_ID) {
    ethOpts['chainId'] = parseInt(process.env.DEPLOY_CHAIN_ID);
}
...
(async () => {
    ...
    const wallet = args.deployerPrivateKey ? new Wallet(args.deployerPrivateKey, provider) :
        Wallet.fromMnemonic(process.env.MNEMONIC, "m/44'/60'/0'/0/1").connect(provider);

    const gasPrice = 0;//args.gasPrice ? parseUnits(args.gasPrice, "gwei") : await provider.getGasPrice();
    ...
    if (args.contract === "ZkSync" || args.contract == null) {
        await deployer.deployZkSyncTarget({gasPrice, nonce: args.nonce,...ethOpts});
    }

    if (args.contract === "Verifier" || args.contract == null) {
        await deployer.deployVerifierTarget({gasPrice, nonce: args.nonce,...ethOpts});
    }

    if (args.contract === "Governance" || args.contract == null) {
        await deployer.deployGovernanceTarget({gasPrice, nonce: args.nonce,...ethOpts});
    }

    if (args.contract === "Proxies" || args.contract == null) {
        await deployer.deployProxiesAndGatekeeper({gasPrice, nonce: args.nonce,...ethOpts});
    }
})();
