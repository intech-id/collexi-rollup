...
impl<'a> AccountSchema<'a> {
    ...
    pub fn last_committed_state_for_account(
        ...
        let (account_tokens_diff, account_creation_diff) = self
            .0
            .conn()
            .transaction::<_, diesel::result::Error, _>(|| {
                // From `account_balance_updates` load entries with the same ID and height
                // greater than for the last verified block.
                let account_tokens_diff: Vec<StorageAccountUpdate> = {
                    account_tokens_updates::table
                        .filter(account_tokens_updates::account_id.eq(&(i64::from(account_id))))
                        .filter(account_tokens_updates::block_number.gt(&last_block))
                ...
                Ok((account_tokens_diff, account_creation_diff))
            })?;
    ...
    fn get_account_and_last_block(
        ...
        if let Some(account) = maybe_account {
            let tokens: Vec<StorageToken> =
                StorageToken::belonging_to(&account).load(self.0.conn())?;

            let last_block = account.last_block;
            let (_, account) = restore_account(account, tokens);
